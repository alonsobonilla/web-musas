...
def api_eliminarcategoria():
    try:
        ...
        return jsonify({"Mensaje": "Categoria elimada correctamente"})
        return
    except:
...
def api_insertarcategoria():
    try:
        ...
        return jsonify({"Mensaje":"Categoria registrada correctamente"})
    except:
...
def api_actualizarcategoria():
    try:
        ...
        return jsonify({"Mensaje":"Error al actualizar categoria"})


@api_categoriaProducto.route("/api_obtenercategoriaid")
def api_obtenercategoriaid():
    try:
        id = request.json["idCategoria"]
        categorias=CategoriaProducto.obtener_categoria_por_id(id)
        listaserializable = []
        for categoria in categorias:
          miobj = CategoriaProducto(categoria[0],categoria[1],categoria[2])
          listaserializable.append(miobj.midic.copy())
